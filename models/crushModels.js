...
const validName = (name) => {
  ...
  if (String(name).length < 3) throw new Error('O "name" deve ter pelo menos 3 caracteres');
};

const validAge = (age) => {
  if (age === undefined) throw new Error('O campo "age" é obrigatório');

  if (Number(age) < 18) throw new Error('O crush deve ser maior de idade');
};

const validDate = (datedAt) => {
  const reGex = /^([0-2][0-9]|(3)[0-1])(\/)(((0)[0-9])|((1)[0-2]))(\/)\d{4}$/;
  if (!reGex.test(datedAt)) throw new Error('O campo "datedAt" deve ter o formato "dd/mm/aaaa"');
};

const validaDatedAtIsValid = (element) => {
  if (!element.date.datedAt || element.date.datedAt === undefined) return true;
};
